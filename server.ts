...
async function startServer() {
  ...
  // Dibutuhkan agar alamat IP pengunjung terbaca dengan benar di balik proxy hosting.
  app.set('trust proxy', 1);

  app.use(express.json({ limit: '1mb' }));

  // Rate limit sederhana untuk melindungi kuota Gemini API dari penyalahgunaan.
  // Untuk skala besar, gunakan Redis atau layanan rate limiting terkelola.
  ...
  function aiRateLimiter(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    const recentRequests = (aiRequestLog.get(clientIp) || []).filter(
      (timestamp) => now - timestamp < AI_RATE_WINDOW_MS,
    );

    if (recentRequests.length >= AI_RATE_LIMIT) {
      return res.status(429).json({
        error: 'Batas penggunaan analisis AI tercapai. Silakan coba lagi beberapa menit.',
      });
    }

    recentRequests.push(now);
    aiRequestLog.set(clientIp, recentRequests);
    return next();
  }
  ...
  app.post('/api/pollution/ai-analysis', aiRateLimiter, async (req, res) => {
    try {
      if (!ai) {
        return res.status(503).json({
          error: 'Fitur AI belum aktif karena GEMINI_API_KEY belum dikonfigurasi pada server.',
        });
      }
